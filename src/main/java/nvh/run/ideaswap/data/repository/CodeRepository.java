...
public interface CodeRepository extends MongoRepository<Codes,String> {
    Codes findCodesByCode(@NotBlank(message = "Code cannot be empty") int code);
    @Query("{ 'code' : ?0, 'userEmail' : ?1, 'codeExpiration' : { $gt: ?2 } }")
    Optional<Codes> findValidCode(@Param("code") int code, @Param("userEmail") String userEmail, @Param("now") Date now);
