...
public class FollowController {
    ...
    public ResponseEntity<Object> deleteFollow(@RequestBody FollowRequest followRequest) {
        return ResponseEntity.ok(
                Map.of(
                        "success", true,
                        "message", "Follow deleted successfully",
                        "follow",followService.deleteFollow(followRequest)
                )
        );
    }

    @DeleteMapping("/delete/byId/{id}")
    public ResponseEntity<Object> deleteFollowById(@PathVariable String id) {
        return ResponseEntity.ok(
                Map.of(
                        "success", true,
                        "message", "Follow deleted successfully",
                        "follow",followService.deleteFollowById(id)
                )
        );
    }
